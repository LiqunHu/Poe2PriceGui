...
```
POE2 物价补丁GUI版本: 为《Path of Exile 2》自动抓取通货物价，查询装备价值工具(实时物价指的是打入补丁那一刻的物价，刷新物价需要自己手动更新，也就是得重新打补丁。)。
...
```
v1.0.12
1.删除默认Token，Token加入验证功能。
2.优化泥人补丁屏蔽效果。
3.添加过滤器GUI控制功能，目前开发进度功能未完善(未测试)。
4.优化泥人补丁备份路径在默认路径下，移植到AppData目录下，防止软件更新备份丢失问题。

v1.0.11
1.修复价格补丁生成时，价格显示神圣石出现[1.xd]的情况，优化价格计算。
2.新增国际服查价测试按钮，支持繁体中文物品文本解析。
3.优化游戏目录检测，新增GGG官方注册表、快捷方式扫描等多种路径查找方式。
4.新增英文词缀缓存，支持国际服交易API英文搜索条件生成。
5.新增技能补丁刷新按钮，完善补丁管理UI。
6.新增词缀解析调试日志，优化语言切换逻辑。
7.补充异界地图标记文件保护规则。
8.添加怪物血条补丁，支持显示怪物血条。
9.优化泥人补丁特效默认屏蔽逻辑，将亡语系列特效显示出来(未测试)。

v1.0.10
1.优化默认特效泥人中深渊黑雾显示，部分爆炸等效果显示.
2.新增技能特效补丁管理功能，支持加载补丁文件、应用补丁和还原操作，自动适配游戏GGPK/Bundles2格式，添加目录快捷打开按钮，优化补丁操作后的内存回收。

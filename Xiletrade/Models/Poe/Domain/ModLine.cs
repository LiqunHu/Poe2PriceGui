...
internal sealed record ModLine
{
    ...
    private int GetAffixIndex(AffixFlag affix, ItemFlag item)
    {
        ...
        // 国服 zh-CN 数据文件无 augment 类别，rune 词缀只匹配到 explicit 条目。
        // 当 affix.Rune=true 且 AffixList 中无 augment 条目时，注入合成"符文"条目，
        // 让下方 TrySelect(General145_Augment, affix.Rune) 能正确命中，rune 词缀显示为"符文"而非"外延"。
        if (affix.Rune && AffixList.Count > 0)
        {
            bool hasAugment = false;
            for (int i = 0; i < AffixList.Count; i++)
            {
                if (AffixList[i].Name.AsSpan().SequenceEqual(Resources.Resources.General145_Augment))
                {
                    hasAugment = true;
                    break;
                }
            ...
            if (!hasAugment)
            {
                var synthId = AffixList[0].ID;
                if (!synthId.StartsWith("augment.", StringComparison.Ordinal))
                {
                    synthId = "augment." + synthId;
                }
                AffixList.Add(new(synthId, Resources.Resources.General145_Augment));
            }
        }

...
internal sealed class ItemData
{
    ...
    private string GetTranslatedName(ItemFlag flag, ReadOnlySpan<char> nameEn)
    {
        ...
        // net8 compat: ReadOnlySpan<char>.Split(char) returning Range[] is net9+.
        string nameEnStr = nameEn.ToString();
        foreach (var wordPart in nameEnStr.Split(' '))
        {
            wordCount++;
            if (_dm.Words.FindWordByNameEn(wordPart.AsSpan()) is var part && part is not null)
            {
    ...
    private string GetEnglishdName(ItemFlag flag, ReadOnlySpan<char> name)
    {
        ...
        foreach (var wordPart in nameStr.Split(' '))
        {
            wordCount++;
            if (_dm.Words.FindWordByName(wordPart.AsSpan()) is var part && part is not null)
            {
